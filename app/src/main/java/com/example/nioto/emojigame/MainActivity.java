    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == INTENT_CREATE_ACTIVITY_KEY){
            if (resultCode == RESULT_OK) {
                String enigma = data.getStringExtra(Constants.EXTRA_BUNDLE_ENIGMA_CREATED_ENIGMA);
                String category = data.getStringExtra(Constants.EXTRA_BUNDLE_ENIGMA_CREATED_CATEGORY);
                FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
                Fragment prev = getSupportFragmentManager().findFragmentByTag(Constants.ENIGMA_CREATED_DIALOG_FRAGMENT_TAG);
                if (prev != null) {
                    ft.remove(prev);
                }
                ft.addToBackStack(null);
                // Create and show the dialog.
                DialogFragment newFragment = EnigmaCreatedDialogFragment.newInstance(enigma, category);
                newFragment.show(ft, Constants.ENIGMA_CREATED_DIALOG_FRAGMENT_TAG);
                //this.updateUIWhenCreating();
            } else if (resultCode == RESULT_OK_UPDATED){
